...
impl <'a> PairMarketStream<'a>{
    ...
    pub fn update_indices(&self) -> Option<()>{
        ...
        let search_start = *self.under_current_index
                                        .as_ref()
                                        .unwrap()
                                        .borrow();
        ...
        let search_start = *self.deriv_current_index
                                        .as_ref()
                                        .unwrap()
                                        .borrow();
        let mut new_deriv_index: Option<usize> = None;
        for i in search_start..self.deriv_data.records.len()-1 {
            if self.deriv_data.records[i].ts <= curr_time && self.deriv_data.records[i+1].ts > curr_time {
...
mod tests {
    ...
    fn create_mock_cb_records(ts_start: u64, ts_end: u64) -> Vec<CoinbaseRecord>{
        ...
        for ts in ts_start..ts_end {
            mock.push(
                CoinbaseRecord::new(ts, 0.0, 0.0, 0.0)
            );
        }
        mock
    }
    ...
    fn test_binary_underlying_records() {
        ...
        let stream = PairMarketStream::new(
            &mock_cb_records[..], 
            mock_kalshi_ds,
            1
        );
