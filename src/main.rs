...
use backtest::strats::gbm_delta_hedge::GBMDeltaHedge;
use backtest::strats::cauch_delta_hedge::CauchyDeltaHedge;
...
fn main() {
    ...
    let init_cash = 1.0;
    let test_func = generate_dh_pair_test_cauch(max_under_pos, min_tte_hedge, init_cash);
...
fn generate_dh_pair_test_gbm(
    max_under_pos: f64,
    min_tte_hedge: f64,
    init_cash: f64,
) -> impl Fn(PairMarketStream) -> TestResult {
    let test = move |stream: PairMarketStream| {
        let strat = GBMDeltaHedge::new(stream, max_under_pos, min_tte_hedge, init_cash);
...
fn generate_dh_pair_test_cauch(
    max_under_pos: f64,
    min_tte_hedge: f64,
    init_cash: f64,
) -> impl Fn(PairMarketStream) -> TestResult {
    let test = move |stream: PairMarketStream| {
        let strat = CauchyDeltaHedge::new(stream, max_under_pos, min_tte_hedge, init_cash);
        let term_port_val = strat.test();
        return term_port_val;
    };
    return test;
