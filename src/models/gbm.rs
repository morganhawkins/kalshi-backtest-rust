...
use statrs::function::erf::{erf_inv, erf};
...
pub fn value(u_price: f64, strike: f64, sigma: f64, mu: f64, tte: f64) -> Option<f64> {
    ...
    let deviations_to_strike = f64::ln(strike/(u_price*tte*mu)) / (f64::sqrt(2.0*tte) * sigma);
...
pub fn delta(u_price: f64, strike: f64, sigma: f64, mu: f64, tte: f64) -> Option<f64> {
    let deviations_to_strike = (f64::ln(strike) - f64::ln(u_price) - (tte*mu) ) / ( f64::sqrt(2.0*tte) * sigma );
...
pub fn vega(u_price: f64, strike: f64, sigma: f64, mu: f64, tte: f64) -> Option<f64> {
    ...
    let vega_ = formula_lhs * ( (expected_units_to_strike) / (sigma.powi(2) * f64::sqrt(2.0 * tte * PI)));
...
pub fn theta(u_price: f64, strike: f64, sigma: f64, mu: f64, tte: f64) -> Option<f64> {
    ...
    let theta_ = formula_lhs * f64::exp(
