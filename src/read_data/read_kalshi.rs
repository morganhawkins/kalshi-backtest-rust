...
pub fn read_kalshi<P: AsRef<Path>>(path: P) -> Result<KalshiDataset, Box<dyn Error>>{
    ...
    let strike_fn = match path.as_ref().file_name() {
        Some(val) => val,
        None => return Err(format!("unable to turn `path` into string: {:?}", path.as_ref()).into()),
    };
    ...
    let strike = match str::parse::<f64>(strike_split_str[0]){
        Ok(val) => val,
        Err(_) => return Err(format!("unable to parse file name w/o extenion into f64 strike price: {:?}", strike_split_str).into()),
    };
    
    let dataset = KalshiDataset{records, strike, start_ts, expir_ts};
    Ok(dataset)
}
