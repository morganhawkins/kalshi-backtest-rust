...
impl KalshiDataset {
    ...
    pub fn relevant_slice<'a>(&self, underlying_data: &'a Vec<CoinbaseRecord>) -> Option<&'a [CoinbaseRecord]>{
...
mod tests {
    ...
    fn create_mock_cb_records(ts_start: u64, ts_end: u64) -> Vec<CoinbaseRecord>{
        ...
        for ts in ts_start..ts_end {
            mock.push(
                CoinbaseRecord::new(ts*2_u64, 0.0, 0.0, 0.0)
            );
        }
        mock
    }
